...
struct uio {
	struct iovec      uio_iovec;       /* Data block */
					   // location and length of the buffer that is being transferred to or from. 
					   // This buffer can be in the application part of the kernel address space or in the kernel's part
	...
	enum uio_seg      uio_segflg;      /* what kind of pointer we have */
					   // the buffer is in the user (application) part of the address space 
					   // or the kernel's part of the address space
	enum uio_rw       uio_rw;          /* whether op is a read or write */
					   // difference: from OR to the file/device
