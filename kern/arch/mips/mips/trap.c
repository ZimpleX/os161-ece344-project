...
mips_trap(struct trapframe *tf)
{
	...
	// mips_trap is called by exception.S,
	// so interrupt should be turned off directly by assembly
	// rather than splhigh(). So curspl is not updated after 
	// interrupt is actually turned off.
	// in the comment of common_exception in exception.S
	// it says that the processor turns off the interrupt for us. 
	curspl = SPL_HIGH;
	...
	// so after mips_trap, there should be a context switch (?)
	// that's why we want to turn off interrupt
	splhigh();
