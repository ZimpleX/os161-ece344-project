...
/*
 * subpage_kmalloc_dumb is only called before coremap_bootstrap
 */
static
void *
subpage_kmalloc_dumb(size_t sz)
{
	int spl;		// saved interrupt level
	unsigned blktype;	// index into sizes[] that we're using
	struct pageref *pr;	// pageref for page we're allocating from
	...
	/*
	 * here is the only difference between the normal & dumb version
	 */
	prpage = alloc_kpages_dumb(1);
...
/*
 * kmalloc_dumb is to be called only in coremap_bootstrap
 * after that, kmalloc will be responsible for update the coremap entry
 * or should we update cmap in alloc_kpages, subpage_kmalloc ?
 */
void *
kmalloc_dumb(size_t sz)
{
	if (sz>=LARGEST_SUBPAGE_SIZE) {
		unsigned long npages;
		vaddr_t address;
		...
		/* Round up to a whole number of pages. */
		npages = (sz + PAGE_SIZE - 1)/PAGE_SIZE;
		address = alloc_kpages_dumb(npages);
		if (address==0) {
			return NULL;
		}
	...
	return subpage_kmalloc_dumb(sz);
}
