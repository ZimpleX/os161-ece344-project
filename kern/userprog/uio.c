...
uiomove(void *ptr, size_t n, struct uio *uio)
{
	...
	while (n > 0 && uio->uio_resid > 0) {
		...
		switch (uio->uio_segflg) {
		    case UIO_SYSSPACE:
			    result = 0;
			    // direction of memmove is opposite for UIO_READ and UIO_WRITE
			    // one is from the file/device; another is to the file/device
			    ...
			    break;
		    case UIO_USERSPACE:	// don't have the priority to move ?
		    case UIO_USERISPACE:
			    if (uio->uio_rw == UIO_READ) {
