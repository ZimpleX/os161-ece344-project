 ...
 * explicitly.
 */
// Wait... where is uiomove called in load_segment?
static
int
...
{
	// elf header: this should appear at the beginning of the elf file
	Elf_Ehdr eh;   /* Executable header */
	// elf program header: for the runtime segment
	...
	// later you will read call mk_kuio with ph: --do iteration through segment ph
	// mk_kuio is to make "kernel" I/O, set uio to UIO_SYSSPACE
	// the 0 arg corresponds to the 0 offset of eh
	...
	for (i=0; i<eh.e_phnum; i++) {
		...
		result = as_prepare_load(curthread->t_vmspace,
					  ...
					  PPAGE_TEMP_FIXED,
					  GETENTRY);
		//kprintf("load vaddr: 0x%08x, memsize: %lu\n", ph.p_vaddr, ph.p_memsz);
		if (result) {
			kprintf("**** loadelf: as_prepare_load fail\n");
			return result;
		}
	}
	/*
	 * we need to load stack manually
	 * now we load the whole stack, but later we might want to change it. 
	 */
	//vaddr_t stackbase = 0x80000000 - PAGE_SIZE * DUMBVM_STACKPAGES;
	//as_prepare_load(curthread->t_vmspace, stackbase, 
	//		DUMBVM_STACKPAGES*PAGE_SIZE, 1, 1, 0);
	/*
